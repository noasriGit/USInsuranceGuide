---
stateSlug: washington-dc
categorySlug: business-insurance
metaTitle: "D.C. Business Insurance Guide"
metaDescription: "Overview of D.C. business insurance — workers' comp at one or more employees, common coverage types, and official DISB resources."
updatedAt: "2025-06-19"
reviewer: content-review-team
sections:
  Overview: |
    Washington, D.C. businesses face a mix of District mandates, federal contracting requirements, contract obligations, and optional coverage. No single policy covers every exposure.

    See our [business insurance in Washington, D.C. guide](/states/washington-dc/business-insurance/) for workers' comp rules, coverage types, and industry considerations.
  Coverage Options to Consider: |
    | Coverage | Often required? |
    |---|---|
    | [Workers' compensation](/blog/workers-compensation-insurance-explained/) | Yes, if you employ 1+ workers |
    | [Commercial auto](/blog/commercial-auto-insurance-explained/) | If you operate business vehicles |
    | [General liability](/blog/general-liability-insurance-explained/) | Contractually; not universal by District law |
    | Commercial property / BOP | Optional; often required by lenders |
    | Professional liability, cyber | By license or contract |

    See also [what insurance a small business may need](/blog/what-insurance-does-a-small-business-need/).
  State Requirements & Regulations: |
    **Workers' compensation:** Employers with **one or more employees** must carry coverage. Domestic workers employed 240+ hours per 13-week quarter may also trigger requirements. Administered by the [DC Office of Workers' Compensation](https://does.dc.gov/page/workers-compensation-does).

    **Commercial auto:** Business vehicles must meet D.C. [auto minimums](/states/washington-dc/auto-insurance/requirements/) (25/50/10).

    **General liability:** Not mandated for all businesses, but common in federal contracts, leases, and client agreements.

    Compare: [Virginia](/states/virginia/business-insurance/) requires workers' comp at **3+ employees** — important if you operate across the DMV.
  Factors That May Affect Your Premium: |
    Industry classification, payroll, claims history, number of employees, vehicles, location, and coverage limits selected. Workers' comp rates vary significantly by trade.
faq:
  - question: "Is workers' compensation required in Washington, D.C.?"
    answer: "Yes, for employers with one or more employees. Homeowners employing domestic workers 240+ hours per 13-week quarter may also need coverage."
  - question: "Is general liability required for D.C. businesses?"
    answer: "The District does not require GL for all businesses, but federal contractors, landlords, clients, and licensing boards often require proof of coverage."
  - question: "Do I need commercial auto insurance in D.C.?"
    answer: "If your business owns or primarily uses vehicles for work, you likely need commercial auto rather than personal coverage."
  - question: "Where do I verify D.C. business insurance rules?"
    answer: "Workers' comp: DC Department of Employment Services. Auto and general insurance: DC DISB and DC DMV."
sources:
  - title: "Workers' Compensation — DC Department of Employment Services"
    url: "https://does.dc.gov/page/workers-compensation-does"
    publisher: "DC Department of Employment Services"
  - title: "DC Department of Insurance, Securities and Banking"
    url: "https://disb.dc.gov/page/insurance"
    publisher: "DC DISB"
  - title: "Business Insurance in Washington, D.C. (full guide)"
    url: "https://usinsuranceguide.com/states/washington-dc/business-insurance/"
    publisher: "US Insurance Guide"
---

<!-- Editorial body lives in frontmatter sections. -->
